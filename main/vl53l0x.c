...
esp_err_t _vl53l0x_get_spad_info(vl53l0x_handle_t vl53l0x, uint8_t *count, bool *aperture) {
  esp_err_t err = _vl53l0x_start_proc(vl53l0x);
  err |= _vl53l0x_write8(vl53l0x, 0xff, 0x06);
  err |= _vl53l0x_update8(vl53l0x, 0x83, 0x04);
  err |= _vl53l0x_write8(vl53l0x, 0xff, 0x07);
  err |= _vl53l0x_write8(vl53l0x, 0x81, 0x01);

  err |= _vl53l0x_write8(vl53l0x, 0x80, 0x01);

  err |= _vl53l0x_write8(vl53l0x, 0x94, 0x6b);
  err |= _vl53l0x_write8(vl53l0x, 0x83, 0x00);

  uint8_t data = 0;
  while (data == 0) // TODO: WARN: That loop could be fatal
    _vl53l0x_read8(vl53l0x, 0x83, &data);
  printf("out of loop 0x83 is 0x%.2x\n", data);
  err |= _vl53l0x_write8(vl53l0x, 0x83, 0x01);

  err |= _vl53l0x_read8(vl53l0x, 0x92, &data);
  *count    = data & 0x7f;
  *aperture = (data & 0x80) >> 7;
  printf("count is %u and type is%s aperture\n", *count, *aperture ? "" : " not");

  err |= _vl53l0x_write8(vl53l0x, 0x81, 0x00);
  err |= _vl53l0x_write8(vl53l0x, 0xff, 0x06);
  err |= _vl53l0x_remove8(vl53l0x, 0x83, 0x04);
  err |= _vl53l0x_write8(vl53l0x, 0xff, 0x01);

  err |= _vl53l0x_stop_proc(vl53l0x);

  return err;
}

esp_err_t vl53l0x_init(vl53l0x_handle_t vl53l0x) {
  esp_err_t err = _vl53l0x_write8(vl53l0x, 0x88, 0x00);
  err |= _vl53l0x_start_proc(vl53l0x);
  uint8_t stop_var;
  err |= _vl53l0x_read8(vl53l0x, 0x91, &stop_var);
  err |= _vl53l0x_stop_proc(vl53l0x);

  err |= _vl53l0x_update8(vl53l0x, MSRC_CONFIG_CONTROL, 0x12);

  err |= _vl53l0x_set_signal_rate_limit(vl53l0x, 0.25);

  err |= _vl53l0x_write8(vl53l0x, SYSTEM_SEQUENCE_CONFIG, 0xff);

  uint8_t spad_count;
  bool    spad_aperture;
  err |= _vl53l0x_get_spad_info(vl53l0x, &spad_count, &spad_aperture);
